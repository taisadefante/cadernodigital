...
type Section = "notes" | "appointments" | "favorites" | "archived" | "calendar";
const defaults: NoteFilters = {
  search: "",
  priority: "all",
  tag: "all",
  dateFrom: "",
...
function message(e: unknown) {
  if (e instanceof FirebaseError && e.code === "permission-denied")
    return "Sem permissão. Confira as regras do Firebase.";
...
async function deleteNoteSubcollection(
  ...
  const snapshot = await getDocs(
    collection(db, "users", userId, "notes", noteId, subcollectionName),
  );
...
async function deleteNoteTree(userId: string, noteId: string) {
  await deleteNoteSubcollection(userId, noteId, "history");

  await deleteNoteSubcollection(userId, noteId, "versions");
...
export default function NotebookApp() {
  ...
  const [notes, setNotes] = useState<Note[]>([]),
    [customTags, setCustomTags] = useState<Tag[]>([]),
    [loading, setLoading] = useState(true),
    [filters, setFilters] = useState(defaults),
    [section, setSection] = useState<Section>("notes"),
    ...
    [deleteTarget, setDeleteTarget] = useState<Note | null>(null),
    [busyDelete, setBusyDelete] = useState(false),
    [toast, setToast] = useState<{
      type: "success" | "danger" | "warning";
      text: string;
    } | null>(null),
    [profile, setProfile] = useState(false),
    [history, setHistory] = useState<Note | null>(null),
    [dark, setDarkState] = useState(false);
  const legacyCleanupRef = useRef<Set<string>>(new Set());
  useEffect(() => {
    const v = localStorage.getItem("caderno-theme") === "dark";
    setDarkState(v);
  }, []);
  const setDark = (v: boolean) => {
    setDarkState(v);
    localStorage.setItem("caderno-theme", v ? "dark" : "light");
  };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
          ...
          s.docs.map(
            (d) =>
              ({
                ...
                ...d.data(),
                tags: d.data().tags || [],
                checklist: d.data().checklist || [],
                recurrence: d.data().recurrence || "none",
                reminderMinutes: d.data().reminderMinutes ?? null,
      ...
      (e) => {
        setToast({ type: "danger", text: message(e) });
        setLoading(false);
      },
    );
    const ut = onSnapshot(
  ...
  useEffect(() => {
    ...
    notes
      ...
      .forEach((note) => {
        if (legacyCleanupRef.current.has(note.id)) return;
        legacyCleanupRef.current.add(note.id);
        void deleteNoteTree(user.uid, note.id).catch(() =>
          legacyCleanupRef.current.delete(note.id),
        );
      });
  }, [notes, user]);
  useEffect(() => {
    ...
    const existing = new Set(
      customTags.map((tag) => tag.name.trim().toLowerCase()),
    );
    ...
    void Promise.all(
      names.map((name) =>
        addDoc(collection(db, "users", user.uid, "tags"), {
          name,
          color: "#64748b",
  ...
  const tagColors = useMemo(
    () =>
      customTags.reduce<Record<string, string>>((acc, tag) => {
        if (tag.name) acc[tag.name] = tag.color || "#64748b";
        return acc;
      }, {}),
    [customTags],
  );
  ...
  const visible = useMemo(
    () => filterNotes(notes, filters, section),
    [notes, filters, section],
  );
  const today = todayIso();
  const storedNotes = notes.filter((n) => !n.deletedAt);
  const active = storedNotes.filter((n) => !n.archived);
  async function historySnapshot(note: Note) {
    if (!user) return;
    await addDoc(
      collection(db, "users", user.uid, "notes", note.id, "history"),
      {
        title: note.title,
  ...
  async function save(data: NoteFormData) {
    ...
    try {
      const payload = {
        ...
        tags: Array.from(
          new Set(data.tags.map((x) => x.trim()).filter(Boolean)),
        ),
        checklist: data.checklist,
        appointment: data.appointment || Boolean(data.date),
        date: data.date || null,
        time: data.date && data.time ? data.time : null,
        recurrence: data.date ? data.recurrence : "none",
        reminderMinutes: data.date ? data.reminderMinutes : null,
      ...
      if (editing) {
        await historySnapshot(editing);
        await updateDoc(
          doc(db, "users", user.uid, "notes", editing.id),
          payload,
        );
        setToast({ type: "success", text: "Anotação atualizada." });
      } else {
        ...
        setToast({ type: "success", text: "Anotação criada." });
      }
      setModal(false);
      setEditing(null);
    } catch (e) {
  ...
  async function patch(note: Note, data: Partial<Note>) {
    if (!user) return;
    try {
      if (data.completed === true && note.recurrence !== "none" && note.date) {
        await historySnapshot(note);
        await updateDoc(doc(db, "users", user.uid, "notes", note.id), {
          completed: false,
          date: addRecurrence(note.date, note.recurrence),
          updatedAt: serverTimestamp(),
        });
        setToast({
          type: "success",
          text: "Compromisso concluído e próxima ocorrência criada.",
        });
        return;
      }
      await updateDoc(doc(db, "users", user.uid, "notes", note.id), {
  ...
  async function archiveNote(note: Note) {
    ...
    try {
      ...
      setToast({
        type: "success",
        text: nextArchived ? "Anotação arquivada." : "Anotação restaurada.",
      });
    } catch (e) {
      setToast({ type: "danger", text: message(e) });
    }
  ...
  async function removeNote() {
    ...
    setBusyDelete(true);
    try {
      await deleteNoteTree(user.uid, deleteTarget.id);
      setDeleteTarget(null);
      setToast({ type: "success", text: "Anotação excluída definitivamente." });
    } catch (e) {
      setToast({ type: "danger", text: message(e) });
    } finally {
      setBusyDelete(false);
  ...
  async function createTag(name: string, color: string) {
    ...
    if (!clean) throw new Error("Informe o nome da tag.");
    if (
      customTags.some(
  ...
  async function updateTagNotes(oldName: string, newName: string | null) {
    if (!user) return;
    ...
    for (let index = 0; index < affected.length; index += 400) {
      const batch = writeBatch(db);
      affected.slice(index, index + 400).forEach((note) => {
        const next = (note.tags || []).flatMap((tag) =>
          tag === oldName ? (newName ? [newName] : []) : [tag],
        );
        batch.update(doc(db, "users", user.uid, "notes", note.id), {
          tags: Array.from(new Set(next)),
          updatedAt: serverTimestamp(),
        });
      });
      await batch.commit();
  ...
  async function renameTag(tag: Tag, newName: string, color: string) {
    if (!user) return;
    ...
    if (
      ...
      throw new Error("Já existe uma tag com esse nome.");
    if (clean !== tag.name) await updateTagNotes(tag.name, clean);
    await updateDoc(doc(db, "users", user.uid, "tags", tag.id), {
      name: clean,
      color: cleanColor,
      updatedAt: serverTimestamp(),
    });
    setFilters((current) =>
      current.tag === tag.name ? { ...current, tag: clean } : current,
    );
  }
  async function deleteTag(tag: Tag) {
    if (!user) return;
    await updateTagNotes(tag.name, null);
    await deleteDoc(doc(db, "users", user.uid, "tags", tag.id));
    setFilters((current) =>
      current.tag === tag.name ? { ...current, tag: "all" } : current,
    );
  }
  async function notifications() {
    ...
    setToast({
      ...
      text:
        p === "granted"
          ? "Lembretes ativados neste navegador."
  ...
  const menu: Array<[Section, string, string]> = [
    ...
    ["archived", "Arquivados", "bi-archive"],
  ];
  ...
  const nextAppointment =
    active
      .filter((note) => {
        if (!note.appointment || !note.date || note.completed) {
          return false;
        }
        ...
        const appointmentDate = new Date(`${note.date}T${note.time}:00`);
  ...
  return (
    ...
    >
      ...
      >
        <div className="container-fluid px-3 px-lg-4 h-100 d-flex align-items-center justify-content-between gap-3">
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <small style={{ color: "#94a3b8" }}>{sectionTitle}</small>
            </div>
          </div>
      ...
      >
        ...
        <aside
          ...
          style={{
            ...
            boxShadow: sidebarHover ? "8px 0 24px rgba(15,23,42,.10)" : "none",
          }}
        >
          <nav
            ...
            <div className="d-grid gap-1">
              {menu.map(([key, label, icon]) => (
                ...
                >
                  ...
                  {sidebarHover && <span className="ms-3">{label}</span>}
                </button>
              ))}
            </div>
          </nav>
          ...
          >
            ...
            >
              ...
              {sidebarHover && <span className="ms-2">Perfil e aparência</span>}
            </button>
            ...
            >
              ...
              {sidebarHover && <span className="ms-2">Sair</span>}
            </button>
          </div>
        </aside>
        ...
        <main className="flex-grow-1" style={{ minWidth: 0 }}>
                    ...
                    style={{
                      ...
                      background: nextAppointment
                        ...
                        : dark
                          ? "#1e293b"
                          : "#fff",
                      color: nextAppointment ? "#d97706" : "#64748b",
                    }}
                  >
                    <i
                      className={`bi ${
                        nextAppointment ? "bi-alarm" : "bi-calendar-check"
                      } fs-5`}
                    />
                  </span>
          ...
          <div className="container-fluid px-3 px-lg-4 py-4">
            <div className="mx-auto" style={{ width: "100%" }}>
              <div className="mb-3">
                <h2 className="fw-bold mb-1">{sectionTitle}</h2>
              ...
              <div className="d-flex justify-content-center mb-3">
                <div className="d-flex flex-wrap align-items-center justify-content-center gap-2 w-100">
                  <div
                  ...
                  {section !== "calendar" && (
                    <button
                      type="button"
                      className={`btn ${
                        filtersOpen ? "btn-primary" : "btn-outline-secondary"
                      }`}
                  ...
                  <div className="dropdown">
                    ...
                    <ul className="dropdown-menu dropdown-menu-end">
                      ...
                      <li>
                        <button
                          className="dropdown-item"
                          onClick={() => exportJson(storedNotes)}
                        >
                  ...
                  <div className="d-flex justify-content-between align-items-center small text-secondary mb-3">
                    ...
                    {visible.length > 0 && (
                      <button
                        ...
                        onClick={() =>
                          void exportNotesToPdf(visible, "Meu Caderno Digital")
                        }
                      >
                    ...
                    >
                      <div className="d-none d-xl-block px-3 px-xl-4 py-2 bg-body-tertiary border-bottom">
                        <div
                          ...
                          style={{ fontSize: 12 }}
                        >
                          <div className="col-xl-4">ANOTAÇÃO</div>

                          <div className="col-xl-2 text-center">
                            TAGS / PRIORIDADE
                          </div>

                          <div className="col-xl-2 text-center">DATA</div>

                          <div className="col-xl-2 text-center">DETALHES</div>

                          <div className="col-xl-2 text-center">AÇÕES</div>
                        </div>
                      </div>
                      ...
                      {visible.map((note, index) => (
                        <NoteCard
                          ...
                          onArchive={(item) => void archiveNote(item)}
                          onPatch={patch}
                          onPdf={(item) =>
                            void exportNotesToPdf(
                              [item],
